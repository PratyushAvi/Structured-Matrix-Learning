...
Condition on $\vec{\Omega}$; then $\vec{Q}$, $\vec{R}$, and every $\vec{B}_i^H$ are fixed, and $\vec{Y}$ is an independent Gaussian matrix.
Each $\hat{\psi}_i$ is an averaged quadratic form with matrix $\vec{N}_i \coloneqq (\vec{B}_i^H)^\T \vec{R}$ and mean $\tr(\vec{N}_i) = \inner{\vec{B}_i^H}{\vec{R}}$.
...
Apply \cref{lem:hw} with $t_i \coloneqq \tfrac{\eps_0}{2}\|\vec{B}_i\|_\F \|\vec{B}\|_\F > 0$; the two exponents satisfy
\[
\frac{k t_i^2}{\|\vec{N}_i\|_\F^2} = \Omega\bigl( \eps_0^4 k^2 \bigr),
\qquad\quad
\frac{k t_i}{\|\vec{N}_i\|_2} = \Omega\bigl( \eps_0^2 k^2 \bigr) = \Omega\bigl( \eps_0^4 k^2 \bigr),
\]
the last step because $\eps_0 \le 1$.
Since $\eps_0^2 k \ge C\sqrt{\log(N/\delta)}$ by \cref{eq:kdef}, both exponents are $\Omega\bigl( C^2\log(N/\delta) \bigr)$.
